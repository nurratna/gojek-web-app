  ...
  describe 'GET #job history' do
    context 'driver logged in' do
      it "assigns the requested view job's driver" do
        driver = create(:driver)
        order1 = create(:order, status: 'Completed', driver_id: driver)
        order2 = create(:order, status: 'Completed', driver_id: driver)
        order3 = create(:order, driver_id: driver)

        get :job, params: { id: driver }
        expect(assigns(:driver)).to eq match_array([order1, order2])
      end
